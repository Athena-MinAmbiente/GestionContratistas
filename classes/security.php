...
class Security
{
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "gestioncontratistas_uggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	*  For AD it is <provider code><username>
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "gestioncontratistas_ugmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "gestioncontratistas_uggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "gestioncontratistas_ugrights", $cman->getUserGroupsConnId() );
	}
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_contractor_master_OwnerID", $userId );
						storageSet( "_contrato_OwnerID", $userId );
						storageSet( "_contractor_master_signature_OwnerID", $userId );
						storageSet( "_informe_intersup_contratista_OwnerID", $userId );
						storageSet( "_informe_intersup_4_OwnerID", $userId );
						storageSet( "_contractor_master_jur_OwnerID", $userId );
						storageSet( "_cert_master_1_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["contractor_doc_id"] );
			storageSet( "_contractor_master_OwnerID", $data["contractor_doc_id"] );
						storageSet( "_contrato_OwnerID", $data["contractor_doc_id"] );
						storageSet( "_contractor_master_signature_OwnerID", $data["contractor_doc_id"] );
						storageSet( "_informe_intersup_contratista_OwnerID", $data["contractor_doc_id"] );
						storageSet( "_informe_intersup_4_OwnerID", $data["contractor_doc_id"] );
						storageSet( "_contractor_master_jur_OwnerID", $data["contractor_doc_id"] );
						storageSet( "_cert_master_1_OwnerID", $data["contractor_doc_id"] );
		}
	}
